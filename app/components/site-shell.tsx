  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(34,211,238,0.16),_transparent_30%),linear-gradient(120deg,_#020617_0%,_#0f172a_45%,_#111827_100%)] px-4 py-6 text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <header className="rounded-[1.25rem] border border-slate-800 bg-slate-900/80 px-4 py-4 shadow-lg shadow-slate-950/30 backdrop-blur sm:px-6">
            ...
            <nav className="flex flex-wrap items-center gap-2">
              {navigation.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="rounded-full border border-slate-700 bg-slate-800/70 px-3 py-2 text-sm text-slate-200 transition hover:border-cyan-400/50 hover:text-white"
                >
        ...
        <section className="rounded-[1.5rem] border border-slate-800 bg-slate-900/80 p-6 shadow-2xl shadow-slate-950/30 backdrop-blur sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <Link href="/" className="rounded-full border border-cyan-400/30 bg-cyan-400/10 px-4 py-2 text-sm font-medium text-cyan-200 transition hover:bg-cyan-400/20">
              🏠 Back to home
            </Link>
          </div>
